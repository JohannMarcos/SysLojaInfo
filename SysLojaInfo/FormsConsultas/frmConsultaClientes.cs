    ...
    public partial class frmConsultaClientes : SysLojaInfo.frmConsulta
    {
        ...
        public void PopulaDataGrid()
        {
            ...
            if (lista.Count > 0)
            {
                ...
                {
                    dgClientes.Rows.Add(item.Codigo, item.nome, item.cpfCnpj, item.rg, item.logr, item.numero, item.natPess, item.bairro, item.limCred, item.aCidade.Codigo, item.aCidade.cidade, item.telRes, item.telCel, item.email, item.aCond.Codigo, item.aCond.condicao, item.DatCad, item.DatUltAlt, item.QuemAlter);
                }
            }
        }
        ...
        public void pegaRegDataGrid()
        {
            ...
            oCliente.aCond.Codigo = Convert.ToInt32(vLinha.Cells["codCond"].Value);
            oCliente.aCond.condicao = Convert.ToString(vLinha.Cells["condicao"].Value);
        ...
        public override void pesquisar()
        {
            ...
            else
            {
                ...
                dgClientes.Rows.Clear();
                dgClientes.Rows.Add(oCliente.Codigo, oCliente.nome, oCliente.cpfCnpj, oCliente.rg, oCliente.logr, oCliente.numero, oCliente.natPess, oCliente.bairro, oCliente.limCred, oCliente.aCidade.Codigo, oCliente.aCidade.cidade, oCliente.telRes, oCliente.telCel, oCliente.email, oCliente.aCond.Codigo, oCliente.aCond.condicao, oCliente.DatCad, oCliente.DatUltAlt, oCliente.QuemAlter);
