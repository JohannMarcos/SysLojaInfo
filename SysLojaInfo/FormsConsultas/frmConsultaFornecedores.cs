    ...
    public partial class frmConsultaFornecedor : SysLojaInfo.frmConsulta
    {
        ...
        public void PopulaDataGrid()
        {
            ...
            if (lista.Count > 0)
            {
                ...
                {
                    dgFornecedores.Rows.Add(item.Codigo, item.nome, item.cpfCnpj, item.rg, item.logr, item.numero, item.natPess, item.bairro, item.aCidade.Codigo, item.aCidade.cidade, item.telRes, item.telCel, item.email, item.aCond.Codigo, item.aCond.condicao, item.DatCad, item.DatUltAlt, item.QuemAlter);
                }
            }
        }
        ...
        public void pegaRegDataGrid()
        {
            ...
            oFornecedor.aCond.Codigo = Convert.ToInt32(vLinha.Cells["codCond"].Value);
            oFornecedor.aCond.condicao = Convert.ToString(vLinha.Cells["condicao"].Value);
